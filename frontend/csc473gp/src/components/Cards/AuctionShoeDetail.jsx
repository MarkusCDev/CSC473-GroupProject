    ...
    useEffect(() => {
        const fetchProducts = async () => {
            try {
                ...
                updateTimer(result.data.creation_date, result.data.shoe_auction_duration);
        ...
        const intervalId = setInterval(() => {
            product && updateTimer(product.creation_date, product.shoe_auction_duration);
        }, 1000);

        return () => clearInterval(intervalId);
    }, [id, product]);

    function updateTimer(creationDate, durationHours) {
        const creationTime = moment(creationDate, "DD/MM/YY HH:mm:ss");
        const currentTime = moment();
        const totalDurationSeconds = durationHours * 3600;
        const elapsed = currentTime.diff(creationTime, 'seconds');
        const timeLeft = Math.max(totalDurationSeconds - elapsed, 0);

        const hours = Math.floor(timeLeft / 3600);
        const minutes = Math.floor((timeLeft % 3600) / 60);
        const seconds = timeLeft % 60;
        setAuctionTimeLeft(`${hours}h ${minutes}m ${seconds}s`);
    }
                        ...
                        <div className="md:flex-1 px-4">
                            ...
                            <div className="flex -mx-2 mb-4">
                                <div className="w-full px-2">
                                    <button onClick={handleAddToCart} className="transition ease-in-out delay-150 hover:scale-110 duration-300 w-1/2 rounded-full bg-blue-700 hover:bg-green-700 text-white w-full text-white bg-blue-500 py-2 px-4 rounded-full font-bold">Add to Cart</button>
                                </div>
                            </div>
                        </div>
                        <div className="md:flex-1 px-4">
                            ...
                            <div className="mb-6">
                                ...
                                <div className="mb-6">
                                    <span className="font-bold text-gray-700 dark:text-gray-300">Time Left:</span>
                                    <span className="text-gray-600 dark:text-gray-300 ml-2">{auctionTimeLeft}</span>
                                </div>
...
export default AuctionShoeDetail;